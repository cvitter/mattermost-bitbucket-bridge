...
def process_payload( hook_path, request_id, data ):
    ...
    if data["eventKey"].startswith( 'pr:' ):
        ...
        url = bitbucket_url + "projects/" + proj_key + "/repos/" + repo_name + "/pull-requests/" + pr_id
    ...
    if data["eventKey"].startswith( 'repo:' ):
        ...
        if  data["eventKey"] == "repo:refs_changed":
            specific_event = "**Action**: " + data["changes"][0]["type"] + " - " +  data["changes"][0]["ref"]["displayId"] + \
                " (" + data["changes"][0]["ref"]["type"] + ")"
            url += "/commits/" +  data["changes"][0]["toHash"]
            attach_extra = specific_event + "\n[Commit: " + data["changes"][0]["toHash"] + "](" + url + ")"
                    
    # Assemble the final attachment text to return and pass to the send_webhook function
